...
function logObserver() {
    ...
    # Exception path /data/persist_log/DCS/en
    DCS_EN_SIZE=`du -s /data/persist_log/DCS/en | awk '{print $1}'`
    traceTransferState "Exception path /data/persist_log/DCS/en size: ${DCS_EN_SIZE}"
    LOG_COUNT_SIZE=`expr ${LOG_COUNT_SIZE} - ${DCS_EN_SIZE}`
